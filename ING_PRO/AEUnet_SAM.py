...
class ReconstructiveSubNetwork(nn.Module):
    def __init__(self,in_channels = 3, out_channels = 3, base_width = 128):
        # , in_channels = 256, out_channels = 3, base_width = 128
        super(ReconstructiveSubNetwork, self).__init__()
#         self.args = args
        self.encoder = EncoderReconstructive(in_channels=3, base_width=128)
        self.decoder = DecoderReconstructive(base_width=128, out_channels=3)

    def forward(self, x):
        b5,b4,b3 = self.encoder(x)
        output = self.decoder(b5,b4,b3)
        return output
